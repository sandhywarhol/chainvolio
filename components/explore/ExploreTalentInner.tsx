...
function ExploreTalentInnerBody({ initialStats }: { initialStats: SiteStats }) {
    const {
                    ...
                    {!loading && total > 0 && (
                        <div className="flex items-center gap-3 pt-4">
                            ...
                            <p className="text-[11px] text-white/30 font-bold uppercase tracking-widest">
                                <span className="text-white/60">34+ Profiles</span> already on the network
                            </p>
                        </div>
                    )}
                </div>
...
// Suspense wrapper — required because useExploreTalent calls useSearchParams
export function ExploreTalentInner({ initialStats }: { initialStats: SiteStats }) {
    return (
        <Suspense fallback={
            ...
            <ExploreTalentInnerBody initialStats={initialStats} />
        </Suspense>
    );
}
