...
export function CustomWalletModal({ isOpen, onClose }: CustomWalletModalProps) {
    ...
    // REQUIREMENT 6: If already connected, do not show the modal content
    if (connected) {
        onClose();
        return null;
    }
    ...
    const handleConnect = async (walletName: string) => {
        try {
            // Pass isMobile so the connection utility knows which path to take
            await connectWallet(walletName, isMobile);
            // On desktop: close modal after successful connect
            // On mobile: the page navigates away, so onClose is a no-op
            onClose();
        } catch {
            onClose();
        }
    ...
    // On mobile, wallets are "available" through deep link even if not detected
        ...
        <div className="fixed inset-0 z-[1000000] flex items-center justify-center p-3 sm:p-4">
            ...
            <div className="relative w-full max-w-[92%] sm:max-w-lg bg-[#0d0d0f] border border-white/10 rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-5 sm:p-8 space-y-6">
                    {/* REQUIREMENT 7: No wallet detected on desktop */}
                            ...
                            >
                                <div className="flex items-center gap-3 sm:gap-5 text-left min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-base sm:text-lg font-bold text-white truncate">{wallet.name}</p>
