...
import { computeCVScore, AttestationTierBreakdown } from "@/shared/logic/cvScore";

// Individual CV Score only.
// For org/DAO Trust Score, use lib/orgScore.ts → calculateOrgTrust().
export async function calculateScore(
    wallet_address: string,
    preFetchedData?: { profile?: any; receipts?: any[] }
) {
    if (!supabaseServer) throw new Error("Supabase internal error");

    const profile = preFetchedData?.profile ?? (
    ...
    const receipts = preFetchedData?.receipts ?? (
        await supabaseServer
            .from("receipts")
            .select("*")
            .eq("wallet_address", wallet_address)
    ...
    const result = computeCVScore(profile, receipts || [], attestationWeights, tierBreakdown);
    ...
    if (error) {
        console.error("[cvScore] upsert failed (ignore if table missing):", error.message);
    }
